...
class AIGenerator:
    ...
    def __init__(self):
        """Initialize Gemini API with API key from config"""
    ...
    def generate_flashcards(self, content, num_cards=10):
        ...
        try:
            prompt = f"""
You are an expert educational content creator. Analyze the following study material and extract the {num_cards} MOST IMPORTANT points that students need to remember.
...
"""
    ...
    def generate_quiz_questions(self, content, num_questions=5):
        ...
        Returns:
            List of dictionaries with question, correct_answer, and options
        """
        try:
            prompt = f"""
You are an expert quiz creator. Generate {num_questions} multiple-choice questions from the following study material.
...
"""
    ...
    def generate_study_summary(self, content, max_length=200):
        ...
        try:
            prompt = f"""
Summarize the following study material in {max_length} words or less. 
...
"""
            ...
            import traceback
            print(f"DEBUG: Recommendation connection error: {e}")
            traceback.print_exc()
            return f"Keep studying! Consistent practice is key to mastery. (Error: {str(e)})"
